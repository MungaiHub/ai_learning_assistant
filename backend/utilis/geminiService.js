...
const generateGeminiText = async (prompt, model = DEFAULT_MODEL) => {
 ...
 * @returns {Promise<Array<{question: string, options: Array, correctAnswer: string, explanation: string, difficulty: string}>>} - Array of quiz questions
 */
...
export const generateQuiz = async (text, numQuestions=5) => {
    ...
    O3: [Option 3]
    O4: [Option 4]
    C: [Correct option - exactly as written as above]
 ...
 *  @returns {Promise<string>} - AI's response to the user's question
 */
export const chatWithContext = async (question, chunks) => {
    const context = chunks.map((c, index) => `[Chunk ${index + 1}]\n${c.content}`).join('\n\n');

    console.log("context_______", context)

    const prompt = `Based on the following context from the document, Analyse the context and answer the user's question accurately and concisely.
...
export const explainConcept = async (concept, context) => {
    ...
    Inncluding examples if relevannt.
